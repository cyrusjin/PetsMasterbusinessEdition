...
async function createOrder(event, openid) {
  ...
  orderData.paymentMode = require('./partnerPayService').ready() && collection.mode === 'online' && collection.state === 'ready' && collection.subMchId && !orderData.placedByMerchant && !orderData.serviceRecord ? 'online' : 'offline';
...
async function updateOrder(event, openid) {
  ...
  const startsBoarding = patch.status === 'boarding'
    && prevStatus !== 'boarding'
    && getOrderServiceLine(existing) !== 'wash'
    && getOrderServiceLine(existing) !== 'homeFeeding';
  let capacityFailure = null;
  const persistUpdate = async () => {
    if (startsBoarding) {
      try {
        await membershipService.assertBoardingCapacity(existing.store_id, 1);
      } catch (err) {
  ...
  if (startsBoarding) {
    await membershipService.withStoreCapacityLock(existing.store_id, persistUpdate);
  } else {
    await persistUpdate();
  }
...
async function handle(event, openid) {
  switch (event.action) {
    ...
    case 'updateOrder': {
      const current = await db.findOne('orders', { order_id: event.order_id || event.id });
      return current && current.paymentMode === 'online'
        ? orderPayment.withOrderLock(event.order_id || event.id, () => updateOrder(event, openid))
        : updateOrder(event, openid);
    }
