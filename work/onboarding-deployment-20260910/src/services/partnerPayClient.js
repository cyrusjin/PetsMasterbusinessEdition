...
function request(method, requestPath, body, multipart) {
  assertEnabled();
  const rawBody = body == null ? '' : JSON.stringify(body);
  const wireBody = multipart ? multipart.buffer : rawBody;
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      headers: {
        ...
        ...(wireBody ? { 'Content-Length': Buffer.byteLength(wireBody) } : {})
      }
    }, (res) => {
      let raw = '';
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            ...
            err.signatureVerified = !!res.headers['wechatpay-signature'];
            throw err;
          }
    ...
    req.on('error', reject);
    if (wireBody) req.write(wireBody);
    req.end();
  });
}

function encryptSensitive(value) {
  return crypto.publicEncrypt({ key: wechatPayPublicKey(), padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, oaepHash: 'sha1' }, Buffer.from(String(value))).toString('base64');
}

function uploadMedia(buffer, filename = 'material.jpg') {
  if (!Buffer.isBuffer(buffer) || buffer.length > 5 * 1024 * 1024) throw new Error('图片大小不能超过5MB');
  if (!/^[a-zA-Z0-9_-]+\.(jpg|png|bmp)$/.test(filename)) throw new Error('图片文件名无效');
  const meta = { filename, sha256: crypto.createHash('sha256').update(buffer).digest('hex') };
  const boundary = `PetMaster${randomNonce()}`;
  const head = Buffer.from(`--${boundary}\r\nContent-Disposition: form-data; name="meta"\r\nContent-Type: application/json\r\n\r\n${JSON.stringify(meta)}\r\n--${boundary}\r\nContent-Disposition: form-data; name="file"; filename="${filename}"\r\nContent-Type: image/${filename.endsWith('.jpg') ? 'jpeg' : filename.split('.').pop()}\r\n\r\n`);
  return request('POST', '/v3/merchant/media/upload', meta, { buffer: Buffer.concat([head, buffer, Buffer.from(`\r\n--${boundary}--\r\n`)]), contentType: `multipart/form-data; boundary=${boundary}` });
}
