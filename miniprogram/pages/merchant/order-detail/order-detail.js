...
Page({
  ...
  onGoDaily() {
    ...
    wx.navigateTo({ url: '/pages/merchant/daily-logs/daily-logs?orderId=' + order.id });
  },

  onGoContract() {
    const order = this.data.order;
    if (!order || !order.id) return;
    wx.navigateTo({ url: `/pages/merchant/boarding-contract/boarding-contract?orderId=${order.id}` });
  },
