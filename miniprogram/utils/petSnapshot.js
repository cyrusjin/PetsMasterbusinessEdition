...
function buildPetDetailView(snapshot, orderFallback = {}) {
  ...
  return {
    ...
    character: pet.character || '--',
    personalityName: (pet.personality && pet.personality.typeName) || '',
    personalityShort: (pet.personality && pet.personality.typeShort) || '',
    behaviorHabits: pet.behaviorHabits || '--',
    dietTaboo: pet.dietTaboo || '--',
    specialCare: pet.specialCare || '--',
    remark: pet.remark || '--'
...
function buildContractPetInfoLines(pet, orderFallback = {}) {
  ...
  const merged = {
    ...
    personality: p.personality || snapshot.personality || null,
  ...
  return [
    ...
    { label: '气质类型', value: display(view.personalityName) },
